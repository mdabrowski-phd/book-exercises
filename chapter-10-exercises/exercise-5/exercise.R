...
popular_2013 <- names_2013[names_2013$prop == max(names_2013$prop), "name"]
...
most_popular_in_year <- function(year) {
  ...
  popular_year <- names_year[names_year$prop == max(names_year$prop), "name"]
  popular_year
}
...
number_in_million <- function(name, year) {
  babies_year <- names[names$name == name & names$year == year, "prop"]
  round(babies_year * 1000000, 0)
}
