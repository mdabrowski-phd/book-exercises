...
setwd("~/pskills-dscience/book-exercises/chapter-12-exercises/exercise-1")
...
average_sales <- size_gathered %>% group_by(size) %>% 
  summarize(avg_volume = mean(volume)) %>% arrange(- avg_volume)
...
# the `spread()` function!
by_type_wide <- by_type %>% spread(key = type, value = volume)
